...
public class InstalledFragment extends ListFragment implements ServiceConnection {
    ...
    private void doUninstall(final ApkItem item) {
        ...
        builder.setTitle("Advertencia, ¿seguro que desea eliminarla？");
        ...
        builder.setNegativeButton("borrar", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                if (!PluginManager.getInstance().isConnected()) {
                    Toast.makeText(getActivity(), "El servicio no está conectado", Toast.LENGTH_SHORT).show();
                } else {
                    try {
                        PluginManager.getInstance().deletePackage(item.packageInfo.packageName, 0);
                        Toast.makeText(getActivity(), "eliminar completa", Toast.LENGTH_SHORT).show();
    ...
    public void onViewCreated(View view, Bundle savedInstanceState) {
        ...
        setEmptyText("No instale el tapón");
    ...
    private class MyBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            if (PluginManager.ACTION_PACKAGE_ADDED.equals(intent.getAction())) {
                try {
                  android.util.Log.e("", "mete valores");
                    PackageManager pm = getActivity().getPackageManager();
